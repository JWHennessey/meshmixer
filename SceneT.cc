...
SceneT<M>::softICP(QtModelT<M>* m1, QtModelT<M>* m2)
{
  ...
  while (iterCount <= iterations){
    ...
    runICP(m1, m2, m1SnapRegion, m2SnapRegion, snapSize, iterCount, iterations);
    runICP(m2, m1, m2SnapRegion, m1SnapRegion, snapSize, iterCount, iterations);
    iterCount++;
  }
  return true;
}
...
template <typename M>
bool
SceneT<M>::runICP(QtModelT<M>* m1, QtModelT<M>* m2, std::vector<size_t> m1SnapRegion, std::vector<size_t> m2SnapRegion, double snapSize, int iterCount, int iterations) {
  ...
  //For each point pi in MA Find the local neighborhood N(pi)
  for (typename M::VertexIter v_it=m1->mesh.vertices_begin(); v_it!=m1->mesh.vertices_end(); ++v_it)
  {
    ...
    for (size_t i = 0; i < m1SnapRegion.size(); i++){
      for (size_t j = 0; j < intersection.size(); j++){
        if (m1SnapRegion[i] == intersection[j]){
          ...
          m2->colourFaceFromVertexIndex(correspondVector[i],Point(255,0,0));
    ...
    generateAMatrix(A, qHat, pHat);
    Matrix<double, 3, 3> H(3,3);
    for (int d = 0; d < pHat.rows(); d++){
      H += pHat.row(d).transpose() * qHat.row(d);
    }
    JacobiSVD< MatrixXd > svd(H, ComputeThinU | ComputeThinV);
    
    Matrix<double, 3, 3> R = svd.matrixV() * svd.matrixU().transpose();
    Matrix<double, 1, 3> temp1 = (R * localMean.transpose());
    Matrix<double, 1, 3> temp2 = coreMean;
    Matrix<double, 1, 3> t = coreMean - localMean;
  ...
  for (typename M::VertexIter v_it=m1->mesh.vertices_begin(); v_it!=m1->mesh.vertices_end(); ++v_it)
  {
    Matrix<double, 1, 3> S = scalings[it];
    ...
    Rquat = Eigen::Quaterniond::Identity().slerp(li, Rquat);
    
    //Do scaling then rotation then translation?
    Point vertex = m1->mesh.point(*v_it);
    Eigen::Vector3d p = Eigen::Vector3d(vertex[0], vertex[1], vertex[2]);
    
    
    if (!isnan(R(0,0))) p = Rquat.matrix() * p;
    p = p + Eigen::Vector3d(T(0,0),T(0,1),T(0,2));
    //p = Eigen::Vector3d(p[0]+S[0], p[1]+S[1], p[2]+S[2]);

    m1->mesh.set_point( *v_it, Point(p[0], p[1], p[2]));
    
    it++;
  }
  //recalculate boundary vertices
  m1->findBoundaryVertices();
  m2->findBoundaryVertices();
}

template <typename M>
double
getDistanceToBoundary(QtModelT<M>* m1, std::vector<double> query_pt, int iterCount, double snapSize){
  typedef nanoflann::KDTreeEigenMatrixAdaptor< PointMatrix > my_kd_tree_t;
  my_kd_tree_t b_mat_index(3, m1->boundaryMatrix, 10);
  b_mat_index.index->buildIndex();
  ...
  int elasticity = 1;
  //calculate distance to snapping region
  std::vector<size_t> ret_index(m1->boundaryMatrix.rows());
  std::vector<double> out_dist_sqr(m1->boundaryMatrix.rows());
  nanoflann::KNNResultSet<double> resultSet(m1->boundaryMatrix.rows());
...
findCorrespondence(QtModelT<M>* m1, QtModelT<M>* m2, std::vector<size_t> m1SnapRegion, std::vector<size_t> m2SnapRegion){
  ...
  for (size_t i = 0; i < m1SnapRegion.size(); i++){
    ...
    for (size_t j = 0; j < m2SnapRegion.size(); j++){
      ...
      float gaussdist = m1->mesh.data(m1Handle).gauss() - m2->mesh.data(m2Handle).gauss();
      float wdist = 0.6 * euclidist + 0.2 * normdist + 0.2 * gaussdist;
      if (i < 10) std::cout << wdist << ": " << euclidist << " " << normdist<< " " << gaussdist << "\n";
      if (wdist < closest){
        closest = wdist;
        correspond = j;
      }
    }
    correspondVector.push_back(m2SnapRegion[correspond]);
  }
...
SceneT<M>::computeSnapRegion(QtModelT<M>* m1, float snapMax)
{
  ...
  for(size_t i=0; i < m1->boundaryPoints.size(); i++)
  {
    ...
    for (size_t j = 0; j < count; j++){
      m1->colourFaceFromVertexIndex(resultPairs[j].first, Point(255,255,255));
