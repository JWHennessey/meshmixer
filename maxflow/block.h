...
template <class Type> class MyBlock
{
public:
  ...
  MyBlock(int size, void (*err_function)(char const*) = NULL) { first = last = NULL; block_size = size; error_function = err_function; }

  /* Destructor. Deallocates all items added so far */
  ~MyBlock() { while (first) { block *next = first -> next; delete[] ((char*)first); first = next; } }
 ...
 private:
  ...
  void	(*error_function)(char const*);
};
...
template <class Type> class DBlock
{
 public:
  /* Constructor. Arguments are the block size and
     ...
     will be called if allocation failed; the message
     passed to this function is "Not enough memory!" */
  DBlock(int size, void (*err_function)(char const*) = NULL) { first = NULL; first_free = NULL; block_size = size; error_function = err_function; }
