...
template < typename M, typename captype, typename tcaptype, typename flowtype > class Graph {
 public:
  typedef enum {
    SOURCE	= 0,
    SINK	= 1
  ...
  Graph(int node_num_max, int edge_num_max, void (*err_function)(char const*) = NULL);
  ...
  flowtype maxflow(bool reuse_trees = false, MyBlock<node_id>* changed_list = NULL);

  // After the maxflow is computed, this function returns to which
  // segment the node 'i' belongs (Graph<M, captype,tcaptype,flowtype>::SOURCE or Graph<M, captype,tcaptype,flowtype>::SINK).
  //
 ...
 private:
  ...
  void	(*error_function)(char const*);	// this function is called if a error occurs,
...
template < typename M, typename captype, typename tcaptype, typename flowtype> 
  inline typename Graph<M, captype,tcaptype,flowtype>::node_id Graph<M, captype,tcaptype,flowtype>::add_node(int num)
{
  assert(num > 0);
...
template < typename M, typename captype, typename tcaptype, typename flowtype> 
  inline void Graph<M, captype,tcaptype,flowtype>::add_tweights(node_id i, tcaptype cap_source, tcaptype cap_sink)
{
...
template < typename M, typename captype, typename tcaptype, typename flowtype> 
  inline void Graph<M, captype,tcaptype,flowtype>::add_edge(node_id _i, node_id _j, captype cap, captype rev_cap)
{
...
template < typename M, typename captype, typename tcaptype, typename flowtype> 
  inline typename Graph<M, captype,tcaptype,flowtype>::arc* Graph<M, captype,tcaptype,flowtype>::get_first_arc()
{
  return arcs;
}

template < typename M, typename captype, typename tcaptype, typename flowtype> 
  inline typename Graph<M, captype,tcaptype,flowtype>::arc* Graph<M, captype,tcaptype,flowtype>::get_next_arc(arc* a) 
{
  return a + 1; 
}

template < typename M, typename captype, typename tcaptype, typename flowtype> 
  inline void Graph<M, captype,tcaptype,flowtype>::get_arc_ends(arc* a, node_id& i, node_id& j)
{
...
template < typename M, typename captype, typename tcaptype, typename flowtype> 
  inline tcaptype Graph<M, captype,tcaptype,flowtype>::get_trcap(node_id i)
{
  assert(i>=0 && i<node_num);
...
template < typename M, typename captype, typename tcaptype, typename flowtype> 
  inline captype Graph<M, captype,tcaptype,flowtype>::get_rcap(arc* a)
{
  assert(a >= arcs && a < arc_last);
  return a->r_cap;
}
...
template < typename M, typename captype, typename tcaptype, typename flowtype> 
  inline void Graph<M, captype,tcaptype,flowtype>::set_trcap(node_id i, tcaptype trcap)
{
  assert(i>=0 && i<node_num); 
...
template < typename M, typename captype, typename tcaptype, typename flowtype> 
  inline void Graph<M, captype,tcaptype,flowtype>::set_rcap(arc* a, captype rcap)
{
  assert(a >= arcs && a < arc_last);
  a->r_cap = rcap;
}
...
template < typename M, typename captype, typename tcaptype, typename flowtype> 
  inline typename Graph<M, captype,tcaptype,flowtype>::termtype Graph<M, captype,tcaptype,flowtype>::what_segment(node_id i, termtype default_segm)
{
  if (nodes[i].parent)
    {
...
template < typename M, typename captype, typename tcaptype, typename flowtype> 
  inline void Graph<M, captype,tcaptype,flowtype>::mark_node(node_id _i)
{
  node* i = nodes + _i;
  if (!i->next)
    {
...
#if defined(OM_INCLUDE_TEMPLATES) && !defined(GRAPH_CPP)
